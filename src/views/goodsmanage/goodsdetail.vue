<template>
  <div class="container" v-loading="fullscreenLoading" :element-loading-text="loadingTxt">
    <el-form action="" ref="form" :model="form" :rules="rules">
      <el-row>
        <el-col :span="6">
        ...
        <el-col :span="6">
          <el-form-item prop="templateId">
            <div>*运费模板：</div>
            <el-select placeholder="请选择运费模板" v-model="form.templateId" filterable>
              <el-option
      ...
      <el-row>
        ...
        <el-col :span="6">
          <el-form-item prop="buyNumVirtual">
            ...
            <!--<el-input type="number" v-model="form.buyNumVirtual" style="width: 220px"></el-input>-->
            <div class="el-input" style="width: 220px;">
              <input type="text" class="el-input__inner" maxlenght=50 v-model="form.buyNumVirtual"
                     @blur="toSetNumber(form.buyNumVirtual, form,'buyNumVirtual', false)"
                     @input="moneyInput(form.buyNumVirtual, form,'buyNumVirtual', false)">
            </div>
          </el-form-item>
        </el-col>
        <el-col :span="6">
          <el-form-item prop="sn">
            <div>货号：</div>
            <el-input type="text" :maxlength="50" placeholder="请输入货号" v-model="form.sn"
      ...
      <el-row>
        ...
        <el-col :span="6">
          <el-form-item prop="brief">
            <div>会员折扣：</div>
            <el-radio-group v-model="form.use_coupon" >
      ...
      <div class="goods-size">
        ...
        <!--<div class="discount">-->
          <!--<el-radio-group v-model="radio" @change="changeRadio">-->
            <!--<el-radio :label="1" v-model="radio">有优惠</el-radio>-->
            <!--<el-radio :label="0" v-model="radio">无优惠</el-radio>-->
          <!--</el-radio-group>-->
          <!--<el-tooltip effect="dark" content="商品选择无优惠可享受会员折扣，选择有优惠不享受折扣，以优惠价进行结算。"-->
        ...
        <el-table
          ...
          <el-table-column
            ...
            <template scope="scope">
              ...
              <!--<el-input
                  @input="moneyInput(scope.row.price, scope.$index, 'price', true)"
                      size="small"
                      v-model="scope.row.price"
              ></el-input>-->
              <!--<span class="error-tip red"
                    v-show="scope.row.price!==''&&!/^[0-9]+([.]{1}[0-9]{1,2})*$/.test(scope.row.price)">数字(两位小数)</span>-->
            </template>
          </el-table-column>
          <el-table-column
             align="center"
          ...
          <!--<el-table-column-->
             ...
             <!--width="120">-->
            <!--<template scope="scope">-->
              <!--<div class="el-input el-input&#45;&#45;small" :class="{'is-disabled': isDiscounts}">-->
                <!--<input type="text" class="el-input__inner" :disabled="isDiscounts" v-model="scope.row.promotionPrice"-->
                       <!--:class="{'is-error': scope.row.promotionPrice===''&& !isDiscounts }"-->
                       <!--@blur="toSetNumber(scope.row.promotionPrice, scope.row, 'promotionPrice', true, true)"-->
                       <!--@input="moneyInput(scope.row.promotionPrice, scope.row, 'promotionPrice', true)">-->
              <!--</div>-->
              <!--<span v-show="scope.row.promotionPrice===''&& !isDiscounts" class="error-tip red">必填</span>-->
              <!--&lt;!&ndash;<span class="error-tip red"-->
                    <!--v-show="scope.row.promotionPrice!==''&&!/^[0-9]+([.]{1}[0-9]{1,2})*$/.test(scope.row.promotionPrice)&&isDiscounts==false">数字(两位小数)</span>&ndash;&gt;-->
            <!--</template>-->
          <!--</el-table-column>-->
          <el-table-column
             align="center"
            ...
            <template scope="scope">
              ...
              ></el-input>-->
              <!--<span class="error-tip red"
                    v-show="scope.row.store!==''&&!/^[0-9]*$/.test(scope.row.store)">须为正自然数</span>-->
            </template>
          </el-table-column>
          <!--<el-table-column
              align="center"
            ...
            <template scope="scope">
              <div class="el-input el-input&#45;&#45;small">
                <input type="text" class="el-input__inner" v-model="scope.row.limitNum"
                       @blur="toSetNumber(scope.row.limitNum, scope.row, 'limitNum', false)"
                       @input="moneyInput(scope.row.limitNum, scope.row, 'limitNum', false)">
              </div>
            </template>
          </el-table-column>-->
          <el-table-column
          ...
          <el-table-column
             ...
             width="100">
            <template scope="scope">
              <el-button @click.native.prevent="delGoodsSize(scope.$index)" icon="delete" size="small">
                删除
              </el-button>
            </template>
          </el-table-column>
        ...
        <el-table
          ...
          <el-table-column
            ...
            <template scope="scope">
              <el-input
                 size="small"
                 v-model="scope.row.propertyKey"
              ></el-input>
              <!--<span class="error-tip red" v-show="scope.row.propertyKey===''">该内容不能为空</span>-->
            </template>
          </el-table-column>
          <el-table-column
             align="center"
            ...
            <template scope="scope">
              ...
              ></el-input>
              <!--<span class="error-tip red" v-show="scope.row.propertyValue===''">该内容不能为空</span>-->
            </template>
          </el-table-column>
          <el-table-column
             align="center"
             ...
             width="200">
            <template scope="scope">
              <el-button @click.native.prevent="delGoodsAttr(scope.$index)" icon="delete" size="small">
                删除
              </el-button>
            </template>
          </el-table-column>
      ...
      <div class="goods-image">
        <div class="title">*商品主图：（支持 JPG、PNG、GIF格式，最多5张，5MB以内，建议尺寸750*750，首张图片为商品主图。）</div>
    ...
    <el-dialog
       class="dialogSpecSelect"
       title="请选择规格"
       :show-close="false"
       :visible.sync="dialogSpec">
      ...
      return {
        ...
        rules: {
          name: [
            ...
            {min: 0, max: 50, message: '长度不超过50字符', trigger: 'blur'},
          ],
          brief: [
            {required: false, message: '请输入商品简介', trigger: 'change'},
            {min: 0, max: 50, message: '长度不超过50字符', trigger: 'blur'}
          ],
          sn: [
            {required: false, message: '请输入货号', trigger: 'change'},
            {min: 0, max: 50, message: '长度不超过50字符', trigger: 'blur'}
          ]
        },
        // 品牌选项的传参
...
//          {specItemList: []}
    ...
    created() {
      ...
      if (goodsId > -1) {
        ...
        getGoodsDetail(data).then((res) => {
          ...
          this.form.templateId = res.goodsjson.logisticsTemplateId
    ...
    methods: {
      ...
      // 比较销售价和优惠价
      toComparePrice(item) {
        if (!this.isDiscounts) {  // 有优惠价
          console.log(item)
          if (item.price !== '' && item.promotionPrice !== '' && item.price <= item.promotionPrice) {
...
//                            this.imageIndex++
      ...
      changeRadio(value) {
        if (value == 0) {
          ...
          for (let i = 0; i < this.specArray.length; i++) {
            this.specArray[i].promotionPrice = ''
            this.specArray[i].isPromotion = 0
          }
        } else {
          this.isDiscounts = false
          for (let i = 0; i < this.specArray.length; i++) {
            this.specArray[i].isPromotion = 1
            this.specArray[i].promotionPrice = 0.01
          }
        }
      },
      // 显示规格弹窗
      ...
      editMainSpec(index) {
        ...
        /*saveOrEditSpec(reqData).then(res => {
          if (index == -1) {  // 新增
            this.specInfoList.push({
              specId: res.specId,
              specName: this.specInfoSetting.addMainSpecText,
              isChecked: false,
              checkedNum: 0,
              specItemList: []
            })
            ...
            let specData = this.specInfoList[index]
            specData.specName = this.specInfoSetting.changeMainSpecText
            this.specInfoList.splice(index, 1, specData)
            this.specInfoSetting.currentChangeMainSpec = -1
          }
          this.specInfoSetting.changeMainSpecText = ''
          this.specInfoSetting.isCommitting = isCommit
        }).catch(() => {
          this.specInfoSetting.isCommitting = isCommit
        })*/
      },
      ...
      toDeleteMainSpec(index) {
        ...
        /*if (this.specInfoList[index].isDelete) {
          return false
        }
        this.specInfoList[index].isDelete = true
        let reqData = {
          token: this.$store.getters.token,
          shopId: this.$store.getters.shopId,
          specId: this.specInfoList[index].specId
        }
        deleteSpec(reqData).then(res => {
          this.specInfoList[index].isDelete = false
          this.specInfoList.splice(index, 1)
        }).catch(() => {
          return false
        })*/
      ...
      // 添加规格属性
      toAddSpecItem() {
        /*if (this.specInfoSetting.isCommittingSpecItem) { // 是否正在保存中
          return false
        }*/
        ...
        this.specInfoSetting.isAddSpecItem = false
        this.specInfoSetting.addSpecItemText = ''
        // this.specInfoSetting.isCommittingSpecItem = true
        // let reqData = {
        //   token: this.$store.getters.token,
        //   shopId: this.$store.getters.shopId,
        //   goodsId: (this.$route.query.id > -1 ? this.$route.query.id : ''),
        //   specId: this.specInfoList[this.specInfoSetting.selectMainSpec].specId,
        //   specName: this.specInfoList[this.specInfoSetting.selectMainSpec].specName,
        //   specItemName: this.specInfoSetting.addSpecItemText
        // }
        // 确定  保存规格属性
        // saveSpecItem(reqData).then(res => {
        //   this.specInfoList[this.specInfoSetting.selectMainSpec].specItemList.push({
        //     specItemId: res.specItemId,
        //     specItemName: this.specInfoSetting.addSpecItemText,
        //     specId: this.specInfoList[this.specInfoSetting.selectMainSpec].specId,
        //     isChecked: true
        //   })
        ...
        //   this.specInfoSetting.isAddSpecItem = false
        //   this.specInfoSetting.addSpecItemText = ''
        //   this.specInfoSetting.isCommittingSpecItem = false
        // }).catch(() => {
        //   this.specInfoSetting.isCommittingSpecItem = false
        // })
      },
      ...
      toDeleteSpecItem(index) {
        ...
        //   token: this.$store.getters.token,
        //   shopId: this.$store.getters.shopId,
        //   specItemId: this.specInfoList[this.specInfoSetting.selectMainSpec].specItemList[index].specItemId
        // }
        // deleteSpecItem(reqData).then(res => {
        //   let newItemArray = this.specInfoList[this.specInfoSetting.selectMainSpec]
        //   if (newItemArray.specItemList[index].isChecked) {
        //     newItemArray.checkedNum --
        //   }
        //   newItemArray.specItemList.splice(index, 1)
        //   this.specInfoList.splice(this.specInfoSetting.selectMainSpec, 1, newItemArray)
        //   // this.specInfoList[this.specInfoSetting.selectMainSpec].specItemList.splice(index, 1)
        // }).catch(() => {
        //   return false
        // })
      },
...
//        this.specInfoList[this.specInfoSetting.selectMainSpec]
      ...
      toCreateSpec() {
        ...
        /*this.specInfoList = [
          {"specName": "颜色", "isChecked": false, "checkedNum": 4, "specId": 1,
            "specItemList": [
              {specItemId: 11, specItemName: "蓝色", specId: 1, isChecked: true},
              {specItemId: 12, specItemName: "红色", specId: 1, isChecked: true}
            ]},
          {
            "specName": "尺寸",
            "isChecked": false,
            ...
            "specId": 2,
            "specItemList": [
              {specItemId: 21, specItemName: "28", specId: 2, isChecked: true},
            ]
          },
          {"specName": "大小", "isChecked": false, "checkedNum": 3, "specId": 3,
            "specItemList": [
              {specItemId: 31, specItemName: "大", specId: 3, isChecked: true}
            ]
          }
        ]*/
        ...
        //   shopId: this.$store.getters.shopId,
        //   goodsId: (this.$route.query.id > -1 ? this.$route.query.id : ''),
        //   specArrayStr: JSON.stringify(this.descartes(specMainArray))
        // }
        // this.$http.post('/backend/mobileSellerProductSpec/createSpec.do', reqData).then(res => {
        //   console.log(res)
        //   if (res.body.code == 1) {
        //     let oldSpec = []
        //     oldSpec = oldSpec.concat(this.specArray)
        //     this.specArray = []
        //     res.body.data.newSpecArray.map((newSpec) => {
        //       let obj = {}
        //       if (newSpec.isReplace) {  // 原来是否也是这个名称
        //         let isSame = false
        //         debugger
        //         oldSpec.map((oldSpecItem) => {
        //           if (oldSpecItem.specName == newSpec.specName) {
        //             isSame = true
        //             obj = oldSpecItem
        //           }
        //         })
        ...
        //             specIdAndItemIdArray: newSpec.specIdAndItemIdArray,
        //             sku: '',
        //             price: 0.01,
        //             promotionPrice: this.radio == 0 ? '' : 0.01,
        //             store: 0,
        ...
        this.$refs.form.validate((valid) => {
          if (valid) {
            ...
            let data = {
              ...
              logisticsTemplateId: this.form.templateId,
              name: this.form.name,
